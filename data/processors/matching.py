...
def format_rider_name_for_pcs(full_name: str) -> str:
    ...
    try:
        ...
        logging.warning(f"Could not parse full_name '{full_name}': {e}")
        ...
        try:
            ...
            logging.error(
                f"Fallback normalization failed for '{full_name}': {fallback_e}"
            )
...
def find_best_match(search_string: str, choices: list[str]) -> str | None:
    ...
    for choice in valid_choices:
        try:
            ...
            logging.warning(f"Error comparing '{search_string}' with '{choice}': {e}")
            continue
...
def match_rider_names(
    ...
    if hasattr(riders, "to_dict"):
        ...
        logging.warning(f"Unexpected riders input type: {type(riders)}")
        return {}
